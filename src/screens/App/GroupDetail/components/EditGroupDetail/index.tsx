...
interface EditGroupDetailProps {
    visible: boolean;
    onClose: () => void;
    groupId: string;
...
const EditGroupDetail: React.FC<EditGroupDetailProps> = ({ visible, onClose, groupId, fetchNotifications, notification }) => {
    ...
    const [notificationName, setNotificationName] = useState(notification?.notificationName || "");
    const [loading, setLoading] = useState(false);
    const [isActive, setIsActive] = useState(notification?.status ?? true);
    const [selectedDate, setSelectedDate] = useState(notification?.selectedDate || new Date());
    const [selectedTime, setSelectedTime] = useState(notification?.selectedDate || new Date());
    const [isDatePickerVisible, setIsDatePickerVisible] = useState(false);
    const [isTimePickerVisible, setIsTimePickerVisible] = useState(false);
    const [selectedDay, setSelectedDay] = useState<string>(notification?.dayBefore || '0');
    const [repeatOption, setRepeatOption] = useState(notification?.repeat || '');
    const [filteredRepeatOptions, setFilteredRepeatOptions] = useState<{ label: string; value: string }[]>([]);
    const [photo, setPhoto] = useState<string>(notification?.photo || '');
    const isFormValid =
        !!notificationName &&
        !!selectedDate &&
        !!selectedDay &&
        (filteredRepeatOptions.length === 0 || filteredRepeatOptions.some(opt => opt.value === repeatOption));
    ...
    const getAvailableDayOptions = (selectedDate: Date, repeatOption: string) => {
        // Eğer "no" seçildiyse sadece "Aynı Gün"
    ...
    useEffect(() => {
        if (notification) {
            ...
            setIsActive(notification.status);
            setSelectedDate(notification.selectedDate);
            setSelectedTime(new Date(notification.selectedDate));
            setSelectedDay(notification.dayBefore);
            setRepeatOption(notification.repeat || '');
            setPhoto(notification.photo || '');
        }
    }, [notification]);

    // Day options
    useEffect(() => {
        const availableDays = getAvailableDayOptions(selectedDate, repeatOption).map(opt => opt.value);
    ...
    // Eski fotoğrafı Storage'dan sil
    ...
    const handleCreateNatification = async () => {
        ...
        if (!userId || !notificationName || !selectedDate || !selectedDay) {
        ...
        try {
            ...
            let photoUrl = photo;
            let oldPhotoUrl = notification?.photo || '';
            if (photo && photo !== oldPhotoUrl) {
                // Yeni fotoğraf yüklenecekse eskiyi sil
            ...
            const notificationData = {
                userId,
                groupId,
                createdAt: notification ? notification.createdAt : firestore.FieldValue.serverTimestamp(),
                notificationName,
            ...
            if (notification) {
                ...
                await firestore()
                    ...
                    .doc(notification.notificationsId)
    ...
    return (
        <CModal
            visible={visible}
            onClose={onClose}
            modalTitle={notification ? t('edit_reminder') : t('new_reminder')}
            width={"100%"}
        ...
        >
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.btnConainer}>
                    <CButton
                        title={notification ? t('update_reminder') : t('create_reminder')}
